...
import { parseYoutubeSearchHtml } from '../../src/core/ytScraper.js';
...
export interface TrackSearchMetric {
  query: string;
  track: BenchmarkTrack;
  samples: SearchSample[];
  networkMetrics: BenchmarkMetrics;
  parseMetrics: BenchmarkMetrics;
...
export interface SearchModelResult {
  model: 'html' | 'innertube';
  modelName: string;
  timestamp: string;
  iterationsPerQuery: number;
  trackMetrics: TrackSearchMetric[];
  overallNetworkMetrics: BenchmarkMetrics;
  overallParseMetrics: BenchmarkMetrics;
  overallTotalMetrics: BenchmarkMetrics;
  overallAvgPayloadBytes: number;
}

const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

/**
 * Executes a single HTML search measurement.
 */
export async function executeHtmlSearch(query: string, limit: number = 10): Promise<SearchSample> {
  ...
  if (!res || !res.ok) {
    throw new Error(lastErr ? (lastErr.cause?.message || lastErr.message) : `HTTP Error ${res?.status}`);
  }
...
/**
 * Runs the HTML Search Benchmark across the defined tracks.
 */
export async function runHtmlSearchBenchmark(options: {
  ...
  if (verbose) {
    console.log('===============================================================');
    console.log('  Benchmark de Búsqueda — Modelo Base HTML (www.youtube.com)');
    console.log('===============================================================');
    console.log(`Queries: ${tracks.length} | Iteraciones: ${iterations} | Cooldown: ${cooldownMs}ms\n`);
  }

  // Warmup run (discarded)
  if (verbose) process.stdout.write('🔥 Realizando warmup inicial...');
  try {
    await executeHtmlSearch('Warmup test');
  ...
  for (let i = 0; i < tracks.length; i++) {
    ...
    for (let r = 0; r < iterations; r++) {
      try {
        const sample = await executeHtmlSearch(query, 10);
        samples.push(sample);
  ...
  return {
    model: 'html',
    modelName: 'Modelo Base HTML (Regex ytInitialData)',
